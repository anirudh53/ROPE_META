...
def load_data(
    ...
    for year in training_years:
        if year == training_years[0]:
            loadedT = load_h5(f"{directory}coae_coeffs_{year}.h5")
        else:
            loaded0 = load_h5(f"{directory}coae_coeffs_{year}.h5")
            for key, value in loaded0.items():
                loadedT[key] = np.concatenate((loadedT[key], loaded0[key]), axis=0)
    for year in validation_years:
        if year == validation_years[0]:
            loadedV = load_h5(f"{directory}coae_coeffs_{year}.h5")
        else:
            loaded0 = load_h5(f"{directory}coae_coeffs_{year}.h5")
            for key, value in loaded0.items():
...
class LSTM_Training:
    ...
    def __init__(
        ...
        """initializes LSTM_Training object
                    ...
                    for b in range(x.shape[1]):  # shape is (1, num_samples_in_batch, 16)
                        ...
                        with torch.set_grad_enabled(phase == "train"):
                            ...
                            if np.isnan(loss.detach().cpu().numpy()):
                                print("\n" * 10)
                                print(
                                    f"Loss is {loss.detach().cpu().numpy()}, terminating training"
                                )
                                print("\n" * 10)
                                set_flag = False
                                break
    ...
    def _save_weights(self, best: bool = False) -> None:
        ...
        if best:
            torch.save(self._model.state_dict(), self._path + "best_weights_1.pth")
            with open(self._path + "logs.json", "w+") as f:
                json.dump(self.logs, f)
        else:
            torch.save(self._model.state_dict(), self._path + "final_weights_1.pth")
